...
app = Flask(__name__)
app.secret_key = "maia_investment_grade_2026"
...
def index():
    ...
    <html lang="es">
    <head>
        <title>MAIA - INVESTMENT SCOUT</title>
        <style>
            :root { --neon: #0ff; --pink: #f0f; --green: #0f0; }
            body { background:#000; color:var(--neon); font-family:monospace; margin:0; padding:20px; padding-bottom:100px; }
            ...
            .resumen-global { background: rgba(0,255,255,0.05); border: 1px dashed var(--neon); padding: 15px; margin: 20px 0; }
            .ficha { background:#0a0a0a; border:1px solid #333; padding:25px; margin-top:20px; border-left:5px solid var(--neon); }
            .grid-ficha { display:grid; grid-template-columns: repeat(3, 1fr); gap:15px; margin:15px 0; border-top:1px solid #222; padding-top:10px; }
    ...
    <body>
        <div class="nav">
            <form method="POST" style="display:contents;">
                <button type="submit" name="view_state" value="scout" class="btn {{ 'active' if view == 'scout' }}">SCOUT BANCARIO</button>
        ...
        {% if view == 'scout' %}
            <form method="POST" id="sForm">
                ...
                <button type="button" onclick="go()" class="btn" style="width:100%; margin-top:20px; border-color:var(--green); color:var(--green);">SINCRONIZAR CON MERCADOS DE INVERSIÓN</button>
            </form>

            {% if session['history'] %}
            <div class="resumen-global">
                <h3 style="margin:0; color:var(--pink);">RESUMEN DE NEGOCIOS REALES 2026</h3>
                <p>Análisis consolidado de activos detectados en Bancas de Inversión y Brokers de Energía avanzada.</p>
            </div>
            {% endif %}
            ...
            <div class="ficha">
                ...
                <div class="grid-ficha">
                    ...
                    <div><span class="label">RIESGO MERCADO</span>{{ r.riesgo }}</div>
                ...
                <div style="background:#111; padding:15px; border:1px solid #222;">
                    <span class="label">RESUMEN EJECUTIVO DEL PROYECTO</span>
                    <p style="color:#ccc; font-size:12px;">{{ r.resumen }}</p>
                    <a href="{{ r.fuente }}" target="_blank" style="color:var(--pink);">[ACCEDER A LA FUENTE BANCARIA/REAL]</a>
                </div>
        ...
        <div id="maia-chat">
            <div class="chat-h" onclick="toggle()"><span>MAIA CORE</span><span id="c-ico">[-]</span></div>
            <div id="chat-b">
                > Conexión a Fuentes de Inversión (Reuters/Bloomberg).<br>
                > Filtrado de Negocios 100% Real.<br>
                > Campos Fijos y Chat Flotante Activo.
            </div>
        </div>
        ...
        <script>
            function toggle() {
                ...
                var i = document.getElementById('c-ico');
            ...
            function go() {
                ...
                var i=setInterval(function(){ w+=10; b.style.width=w+'%'; if(w>=100){ clearInterval(i); document.getElementById('sForm').submit(); }}, 100);
            }
        </script>
    </body></html>
    """
