...
app = Flask(__name__)
app.secret_key = "maia_global_full_2026"
...
def index():
    ...
    <html lang="es">
    <head>
        <title>MAIA - GLOBAL 360 FULL</title>
        <style>
    ...
    <body>
        <div class="nav">
            <form method="POST" style="display:contents;">
                <button type="submit" name="view_state" value="scout" class="btn {{ 'active' if view == 'scout' }}">SCOUT GLOBAL</button>
        ...
        {% if view == 'scout' %}
            <form method="POST" id="scoutF">
                ...
                <button type="button" onclick="go()" class="btn" style="width:100%; margin-top:20px; border-color:var(--green); color:var(--green);">SINCRONIZACIÓN PROFUNDA: AMÉRICA | EUROPA | ASIA | GOLFO</button>
            </form>
        ...
        <div id="maia-chat">
            ...
            <div id="chat-b">
                > Cobertura: América, Europa, Golfo (UAE, QA, SA) y Asia (CN, KR, JP, SG).<br>
                > Ramas: Matriz Energética Completa + Neutrinos + Startups.<br>
                > Fases: 360° (Diseño a Venta de Acciones).<br>
                > Vigencia: 30 días.
            </div>
        </div>
        <script>
