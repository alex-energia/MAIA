...
app = Flask(__name__)
app.secret_key = "maia_fixed_final_2026"
...
def index():
    ...
    <html lang="es">
    <head>
        <title>MAIA - SISTEMA SCOUT</title>
        <style>
    ...
    <body>
        ...
        {% if view == 'scout' %}
            <form method="POST" onsubmit="runLoad()">
                <button type="submit" name="action" value="run_scout" class="btn" style="width:100%; margin-top:20px; border-color:var(--green); color:var(--green);">EJECUTAR RASTREO REAL 2026</button>
            </form>
            ...
            <div class="ficha">
                ...
                <div style="background:#111; padding:20px; border:1px solid #222;">
                    ...
                    <a href="{{ r.fuente }}" target="_blank" style="color:var(--pink); font-size:11px;">[ACCEDER A LA FUENTE ORIGINAL]</a>
                </div>
        ...
        {% elif view == 'memoria' %}
            <h2 style="color:var(--pink);">MEMORIA DE PROYECTOS</h2>
            {% for m in session['saved'] %}
                <div class="ficha"><h3>{{ m.nombre }}</h3><p>{{ m.resumen }}</p></div>
            {% endfor %}
        {% endif %}

        <div id="maia-chat">
            <div class="chat-h" onclick="toggle()">MAIA CORE [-]</div>
            <div id="chat-b">
                > Sistema Restablecido.<br>
                > Barra de Estado Activa.<br>
                > Fichas con 6 Campos Fijos.<br>
                > Memoria y Limpiar Operativos.
            </div>
        </div>
