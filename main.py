...
app = Flask(__name__)
app.secret_key = "maia_level_100_final"
...
def index():
    ...
    <html lang="es">
    <head>
        <title>MAIA - NIVEL 100 (BLACK BOX)</title>
        <style>
    ...
    <body>
        <div class="nav">
            <form method="POST" style="display:contents;">
                <button type="submit" name="view_state" value="scout" class="btn {{ 'active' if view == 'scout' }}">INFILTRACIÓN 100</button>
        ...
        {% if view == 'scout' %}
            <form method="POST" id="scoutF">
                ...
                <button type="button" onclick="start()" class="btn" style="width:100%; margin-top:20px; height:80px; font-size:18px; border-color:var(--neon); color:var(--neon);">Sincronizar Protocolo Black-Box Nivel 100</button>
            </form>

            {% if session['attempt'] and not session['history'] %}
                <div class="alert-null">
                    <p>[ ESCANEO PROFUNDO COMPLETADO ]</p>
                    <p style="font-size:10px;">0 COINCIDENCIAS EN LA CAPA DE SEÑALES DE CAPITAL.</p>
                </div>
            {% endif %}
            ...
            <div class="ficha">
                ...
                <div class="grid">
                    ...
                    <div><span class="label">ANÁLISIS DE SEÑAL</span>{{ r.riesgo }}</div>
                    <div><span class="label">TERMINALES</span>{{ r.movil }}</div>
                    <div><span class="label">E-MAIL INTEL</span>{{ r.email }}</div>
                    <div><span class="label">FECHA</span>{{ r.fecha }}</div>
                </div>
                <div style="background:#000; padding:20px; border:1px solid #111;">
                    <span class="label">DETALLE TÉCNICO</span>
                    <p style="color:#888; font-size:12px; line-height:1.6;">{{ r.resumen }}</p>
                    <a href="{{ r.fuente }}" target="_blank" style="color:var(--neon); text-decoration:none; font-weight:bold; font-size:11px;">[ACCEDER AL REGISTRO]</a>
                </div>
        ...
        <script>
            ...
            function start() {
                ...
                var itv=setInterval(function(){ w+=20; b.style.width=w+'%'; if(w>=100){ clearInterval(itv); document.getElementById('scoutF').submit(); }}, 20);
            }
        </script>
    </body></html>
    """
